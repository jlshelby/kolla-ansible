...
It is also a requirement to define phynet:interface mappings for all
SRIOV devices as shown:
...
However, the provider networks used by SRIOV should be configured.
...
Physical network to interface mappings will be automatically added
from ``neutron_sriov_physnet_mappings`` to
``physical_device_mappings`` under ``sriov_agent.ini``. However,
specific VFs can be excluded here. Leaving blank means to enable all
VFs for the interface:
